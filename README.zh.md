...
> 前端：自然语言交互、现代化又宽容的体验、中英双语叙述 —— 而原版游戏引擎仍是唯一权威。
...
- **macOS** —— 下载 `cavebridge-macos-arm64`，`chmod +x` 后在终端运行。
- **Linux** —— 下载 `cavebridge-linux-x86_64`，`chmod +x` 后运行。
